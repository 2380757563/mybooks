...
def _user_cache_path(uid) -> str:
    return os.path.join(CONF.get("MYREADER_SYNC_PATH", "/data/sync/"), str(uid), "reading.json")
...
def _load_cache(uid) -> Dict:
    ...
    try:
        ...
        logging.warning("[reading_dashboard] failed to load %s, rebuilding from scratch: %s", path, e)
        return _empty_cache()
...
def _query_days(db, reader_id: int, start: datetime.date, end: datetime.date) -> Dict[str, Dict[str, int]]:
...
def _reconcile(db, reader_id: int, cache: Dict, today: datetime.date) -> Dict:
    ...
    if cached_through:
        try:
            cached_through_date = datetime.datetime.strptime(cached_through, "%Y-%m-%d").date()
        except ValueError:
            cached_through_date = None
    ...
    start = (cached_through_date + datetime.timedelta(days=1)) if cached_through_date else (today - datetime.timedelta(days=CACHE_RETENTION_DAYS))
    if start > yesterday:
...
def _weekly_buckets(days: Dict[str, Dict], today: datetime.date) -> list:
    ...
    week_starts = [this_week_start - datetime.timedelta(weeks=i) for i in range(DISPLAY_WEEKS - 1, -1, -1)]
    weekly = []
...
def _book_status(db, reader_id: int) -> Dict[str, int]:
    rows = (
...
def get_stats(db, reader: Reader) -> Optional[Dict]:
    ...
    return {
        ...
        "book_status": _book_status(db, reader.id),
    }
